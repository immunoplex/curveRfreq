...
The structure of `elisa_assay_example`:

A named list of simulated multi-plate ELISA data for a single analyte
(`alpha`), spanning 6 plates and 20 patient samples per plate across 3
timepoints (baseline, month3, month6) and 2 treatment groups. Plates 1–3
use a five-parameter logistic (5-PL) standard curve; plates 4–6 use a
Gompertz curve, reflecting realistic between-plate variability in curve
shape. The assay response column `od` can also be `absorbance`. The
`assay_response_variable` is a string corresponding to the nae of the
assay response column. More information can be found by running
[`help(elisa_assay_example)`](https://immunoplex.github.io/curveRfreq/reference/elisa_assay_example.md).

The `StandardCurve` class expects it to contain `$standards`, `$blanks`,
`$samples`, `$response_var`, and `$indep_var`.
...
#>   ..$ od                        : num [1:60] 0.0955 0.0865 ...
#>   ..$ assay_response_variable   : chr [1:60] "od" "od" ...
...
#>   ..$ od                        : num [1:24] 0.0514 0.0483 0.0516 0.0415 0.0443 ...
#>   ..$ assay_response_variable   : chr [1:24] "od" "od" ...
...
#>   ..$ dilution                  : int [1:120] 400 400 400 400 400 ...
#>   ..$ samplingerrors            : num [1:120] NA NA NA NA NA ...
#>   ..$ od                        : num [1:120] 2.77 3.01 ...
#>   ..$ assay_response_variable   : chr [1:120] "od" "od" ...
...
#>   ..$ study_accession     : chr [1:6] "SDYELISAexample" "SDYELISAexample" ...
